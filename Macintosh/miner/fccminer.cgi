...
if (!-e 'minerwallet.fcc') {
  print "Enter a FCC wallet to mine to: ";
  $WALLET=<STDIN>; chomp($WALLET);
  if (!validwallet($WALLET)) {
    print "> Invalid wallet\n"; exit
  }
  gfio::create("minerwallet.fcc",$WALLET)
} else {
  $WALLET=gfio::content("minerwallet.fcc");
  print "Wallet = $WALLET\n"
}
...
if (-e 'minernode.fcc') {
  ($NODEIP,$NODEPORT)=split(/\:/,gfio::content('minernode.fcc'))
} elsif (!-e 'minerrandom.fcc') {
  print "Node to mine to [ip:port] (Leave blank for random node): ";
  my $node=<STDIN>; chomp($node);
  if (!$node) {
    gfio::create('minerrandom.fcc',1)
  } else {
    if ($node =~ /[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\:[0-9]+/) {
      gfio::create('minernode.fcc',$node);
...
if (-e 'minerrandom.fcc') {
  my $nodes=gclient::website('https://factorialcoin.nl:5151/?nodelist');
  if ($nodes->{content}) {
...
# EOF multithreading FCC miner (C) 2018 Chaosje